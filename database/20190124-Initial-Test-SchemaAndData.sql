...
CREATE TABLE `decks` (
  ...
  PRIMARY KEY (`deckid`),
  UNIQUE KEY `deckid_UNIQUE` (`deckid`),
  KEY `userid_idx` (`deckcreatorid`),
  KEY `idx_decks_deckid` (`deckid`),
  KEY `idx_decks_deckcreatorid` (`deckcreatorid`),
  CONSTRAINT `userid` FOREIGN KEY (`deckcreatorid`) REFERENCES `users` (`userid`)
...
CREATE TABLE `history` (
  ...
  KEY `idx_history_userid` (`userid`),
  KEY `idx_history_historyid` (`historyid`),
  KEY `idx_history_cardid` (`cardid`),
...
CREATE TABLE `userdeckpairing` (
  ...
  UNIQUE KEY `userdeckpairingid_UNIQUE` (`userdeckpairingid`),
  KEY `idx_userdeckpairing_userdeckpairingid` (`userdeckpairingid`),
  KEY `idx_userdeckpairing_deckid` (`deckid`),
  KEY `idx_userdeckpairing_userid` (`userid`)
...
CREATE TABLE `users` (
  ...
  KEY `deckid_idx` (`deckcurrentlyloaded`),
  CONSTRAINT `deckid` FOREIGN KEY (`deckcurrentlyloaded`) REFERENCES `decks` (`deckid`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `usersettings` (
  ...
  UNIQUE KEY `usersettingsid_UNIQUE` (`usersettingsid`),
  KEY `idx_usersettings_usersettingsid` (`usersettingsid`),
...
-- Dump completed on 2019-01-24 13:45:27
